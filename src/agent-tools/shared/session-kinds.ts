...
const PROFILES: Record<SessionKind, SessionKindProfile> = {
  ...
  shell: {
    ...
    hint: ({ ready }) =>
      ready
        ? "[session: shell, prompt detected] Send one shell command per command_poll call, terminated with \\n."
  ...
  reverse_shell: {
    ...
    hint: ({ ready }) =>
      ready
        ? "[session: reverse_shell, prompt detected] This is an interactive remote shell. Send one command per call via command_poll(input=...), each terminated with \\n. Avoid commands that themselves expect further interactive input (e.g. another sub-shell) unless you plan to follow up."
  ...
  repl: {
    kind: "repl",
    looksReady: () => true,
    hint: () => "[session: repl] Send one REPL statement per command_poll call."
  },
  ssh: {
    kind: "ssh",
    looksReady: (chunk) => SHELL_PROMPT_RE.test(chunk),
    hint: ({ ready }) =>
      ready
        ? "[session: ssh, prompt detected] Send one remote command per command_poll call, terminated with \\n."
        : "[session: ssh, no prompt detected] Remote command may still be running; poll again before sending new input."
  },
  oast: {
    kind: "oast",
    looksReady: () => true,
    hint: () => "[session: oast] Use command_poll with no input to check for new DNS/HTTP/SMTP/LDAP interactions. Stop it with command_stop when the test is complete."
  }
};
