  ...
  const stable = [
    ...
    },
    {
      ...
      body: [
        ...
        "For every new security finding, assess the CVSS 3.1 base metrics from observed evidence, then call finding_manage with operation=calculate or operation=add_finding and the complete CVSS:3.1 base vector, target, evidence IDs, impact, reproduction, and remediation. finding_manage persists the candidate in the current session and populates the Findings tab; do not only describe a finding in the final answer. For an existing finding, use operation=update_finding with its findingId to correct one record instead of creating a duplicate. Let Farai derive severity from the calculated score.",
