  ...
  const stable = [
    ...
    },
    {
      ...
      body: [
        ...
        "Prefer purpose-built capabilities over command_run: asset_subdomains and url_discover for passive discovery; dns_resolve, network_scan, service_probe, and tls_inspect for validation and service inventory; web_crawl and web_directory for application mapping; vulnerability_scan and vulnerability_lookup for template scanning, advisory intelligence, or local Exploit-DB lookup; browser_manage for interactive web work; and the knowledge_manage, callback_manage, campaign_manage, finding_manage, and mobile_manage facades for their domains. network_scan performs TCP discovery only unless an explicit Nmap mode is selected. Use command_run for capabilities that genuinely lack a typed tool or for deliberate scripts and advanced Kali workflows; continue long-running commands with command_input.",
    ...
    },
    {
      ...
      body: [
        ...
        "Passive infrastructure discovery is not interactive web exploration. For subdomains, passive DNS, or certificate transparency, call asset_subdomains directly; for historical URLs, use url_discover. Producer and consumer tools are dependency-ordered: never launch dns_resolve, service_probe, tls_inspect, network_scan, or web_crawl in parallel with asset_subdomains when they need its output. Wait for passive discovery, then pass the complete deduplicated discovered set into validation exactly once. Run only genuinely independent tools in parallel.",
