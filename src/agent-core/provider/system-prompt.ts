  ...
  const stable = [
    ...
    },
    {
      ...
      body: [
        ...
        "Passive infrastructure discovery is not interactive web exploration. For subdomains, passive DNS, or certificate transparency, call asset_subdomains directly; for historical URLs, use url_discover. Validate candidates with dns_resolve, service_probe, or network_scan before deeper work, and consume each deduplicated source result once rather than retrying failed sources through equivalent shell variants.",
        ...
        "In explicit proxy mode, browser traffic and http_request are captured automatically while ordinary shell and bulk reconnaissance traffic remain direct. Set command_run network=proxy only when a proxy-aware command must be captured. proxy_scope controls recording only; use proxy_policy for upstream TLS or pass-through behavior. Transparent proxy mode is an explicit fallback for clients that cannot use an HTTP proxy.",
    ...
    },
    {
      ...
      body: [
        ...
        "Use agent_manage only for bounded work that benefits from independent context, parallel I/O, persistent browser state, specialist tools, or independent verification. Use operation=spawn to start a child, list or wait to inspect it, message to steer active work, followup to continue an idle child, and interrupt or close for lifecycle cleanup. Put operation-specific fields inside args. Children inherit the parent model by default; omit model unless an explicit model override is required. Choose the required lane first: explore is read-only without shell; recon has discovery shell; web has browser, HTTP, and shell; code can edit; verify independently checks with browser, HTTP, and shell. Attached work blocks the parent; detached work must be non-editing and independently useful. Give parallel workers non-overlapping ownership, and keep synthesis and the user-facing answer in the parent. For background delegation, pass args.mode=\"detached\"; never invent detached=true or top-level operation arguments.",
